...
import { AlertCircle, ChevronLeft, ChevronRight, X, Calendar as CalendarIcon, User } from 'lucide-vue-next';
...
// Get ALL approved requests for a day (for counting, ignores filters)
...
// Get filtered requests for a day (for display, respects filters)
...
const getTeamCoverage = (date: Date) => {
    ...
    const coveragePercent = totalEmployees > 0 ? (availableCount / totalEmployees) * 100 : 0;
...
<template>
    <Card class="flex-1 border border-slate-200/50 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-sm">
        <CardHeader>
            <div class="flex items-center justify-between">
                <CardTitle class="text-slate-900 dark:text-white">Team Availability Calendar</CardTitle>
        ...
        <CardContent class="space-y-6">
                ...
                <div class="grid grid-cols-7 gap-2">
                    <template v-for="(day, index) in calendarDays" :key="index">
                        ...
                        <button
                            ...
                            :class="
                                cn(
                                    'relative aspect-square rounded-lg border p-2 transition-all border-slate-200/50 dark:border-white/10 cursor-pointer hover:shadow-lg hover:scale-105',
                        ...
                        >
                            <div class="flex h-full flex-col">
                                ...
                                >
                                    ...
                                    >
                                        {{ getTeamCoverage(day).available }}/{{ totalEmployees }}
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-2 text-sm text-slate-600 dark:text-white/60">
                    ...
                    <span>
                        Low coverage (less than 60% available)
                    </span>
                </div>
            </div>
        </CardContent>
    ...
    <Dialog v-model:open="isModalOpen">
        <DialogContent class="flex max-h-[85vh] max-w-4xl flex-col border-white/20 bg-white/90 backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/90">
            <DialogHeader class="shrink-0 border-b border-slate-200/50 pb-4 dark:border-white/10">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
                                Team availability and vacation requests (excluding owners)
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <!-- Scrollable Content -->
            <div class="flex-1 space-y-6 overflow-y-auto py-4 pr-2" style="scrollbar-gutter: stable" v-if="selectedDay">
                <!-- Coverage Stats -->
                <div class="grid gap-4 md:grid-cols-3">
                    <!-- Total Employees -->
                    <div class="group relative overflow-hidden rounded-2xl border border-white/40 bg-white/70 p-4 shadow-lg backdrop-blur-xl transition-all duration-300 hover:scale-[1.02] dark:border-white/20 dark:bg-slate-800/40">
                    ...
                    <div class="group relative overflow-hidden rounded-2xl border border-white/40 bg-white/70 p-4 shadow-lg backdrop-blur-xl transition-all duration-300 hover:scale-[1.02] dark:border-white/20 dark:bg-slate-800/40">
                        ...
                        <div class="relative">
                            ...
                            <p class="mt-1 text-3xl font-bold text-rose-700 dark:text-rose-300">
                ...
                >
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500 shadow-lg">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-red-700 dark:text-red-400">
                            Less than 60% of team members are available on this day
                        </p>
                    </div>
                </div>
                ...
                <div>
                    <h3 class="mb-4 flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white">
                        ...
                        Vacation Requests
                        <span class="ml-2 flex h-6 min-w-[24px] items-center justify-center rounded-full bg-gradient-to-r from-orange-500 to-rose-500 px-2 text-xs font-bold text-white">
                    ...
                    >
                        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
                            <User class="h-8 w-8 text-slate-400 dark:text-slate-500" />
                        </div>
                        ...
                        >
                            ...
                            <div class="relative flex items-center gap-4 p-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mt-1 flex items-center gap-4 text-sm text-slate-600 dark:text-slate-400">
                                        <span class="flex items-center gap-1">
                                            <CalendarIcon class="h-3.5 w-3.5" />
                                            {{ format(request.startDate, 'MMM d') }} - {{ format(request.endDate, 'MMM d, yyyy') }}
                                        </span>
                                        <span v-if="request.department" class="flex items-center gap-1">
                                            {{ request.department }}
                                        </span>
                                    </div>
                                </div>
